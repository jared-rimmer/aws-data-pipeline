...
def test_put_file(s3_client, create_production_s3_bucket, production_bucket_name, set_up_and_tear_down):
    ...
    S3Client(s3_client).put_file(bucket_name=production_bucket_name, file_name='2023-06-29-trades.csv', file_content=file_content)

    result = s3_client.get_object(Bucket=production_bucket_name, Key='2023-06-29-trades.csv')
...
def test_delete_file_by_prefix(s3_client, create_production_s3_bucket, production_bucket_name, set_up_and_tear_down):
    ...
    s3_client.upload_file(first_file, production_bucket_name, 'xyz-987-abc-123-2023-06-29-trades.csv')
    s3_client.upload_file(first_file, production_bucket_name, '2023-06-30-trades.csv')

    S3Client(s3_client).delete_file_by_suffix(bucket_name=production_bucket_name, suffix='2023-06-29-trades.csv', latest_file_name='xyz-987-abc-123-2023-06-29-trades.csv')
