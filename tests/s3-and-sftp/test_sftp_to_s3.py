...
def test_upload_data_from_sftp_to_s3(s3_client, create_s3_bucket, bucket_name):
    ...
    with sftp_connection(get_sftp_connection_credentials()) as sftp_conn:
        for file in files_to_upload:
            SFTPClient(sftp_conn).upload_data_from_sftp_to_s3(
                s3_client,
                file=f'upload/{file["file"]}',
...
def test_versioning_with_upload_data_from_sftp_to_s3(
    s3_client, create_s3_bucket, bucket_name, set_up_and_tear_down
):
    with sftp_connection(get_sftp_connection_credentials()) as sftp_conn:
        SFTPClient(sftp_conn).upload_data_from_sftp_to_s3(
            s3_client,
            file="upload/2023-06-23-trades.csv",
            bucket_name=bucket_name,
            file_name="2023-06-23-trades.csv",
        )
        SFTPClient(sftp_conn).upload_data_from_sftp_to_s3(
            s3_client,
            file="upload/2023-06-23-trades.csv",
    ...
    result = s3_client.list_object_versions(
        Bucket=bucket_name, Prefix="2023-06-23-trades"
    )
